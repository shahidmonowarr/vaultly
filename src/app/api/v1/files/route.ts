...
import { countFiles, getStorageUsage, listFiles, toFileDto } from '@/server/services/files';

export const GET = route(async (request) => {
  const { userId } = await requireUser(request);
  const options = parseQuery(request, listFilesSchema);

  const [page, total, used] = await Promise.all([
  ...
  return json({
    ...
    pagination: { nextCursor: page.nextCursor, limit: options.limit, total },
